...
#eval the CMC/mAP for both benign/adv settings
def single_evaluate(net, probe_path, gallery_path, adv_index, target_index, adv_id, target_id):
    ...
    '''
    query1_f = net.predict(imgs[adv_index], batch_size=128)
    gallery1_f = net.predict(imgs[target_index], batch_size=128)

    query2_f = net.predict(imgs[adv_index], batch_size=128)
    gallery2_f = net.predict(imgs[adv_index], batch_size=128)
    result, result_argsort = sort_similarity(query2_f,gallery2_f )
    '''

    # divide the adv images into 2 part. 1) probe  2)gallery_add
    rand_index = np.arange(len(adv_index))
    #np.random.shuffle(rand_index)
    adv_index = adv_index[rand_index]
    ...
    num_per_pc = 6
    gal_add_pid = sorted(list(set(range(1502,1513))-{adv_id,target_id}))
    otherpid_index = np.random.randint(0, 24, size=3 * (11-2) * num_per_pc) + \
                     np.array([np.arange(3)+(i-1502)*3 for i in gal_add_pid]).flatten().repeat(num_per_pc)*24

    gal_add_index = np.concatenate(
        (adv_index[1:], target_index, otherpid_index)
    )
    ...
    #a. untarget test
    # the probe adv serving as probe
    # gallery is obtained by concat target image, other pid, gallery_add adv
    # do the test:
    result_untarget = eval_without_noise(net, adv_probe_imgs, adv_probe_infos, gallery_imgs_add, gallery_infos_add)

    #b. target test
    # change the adv infos to (target_id, cid)
    # gallery is obtained by concat target image, other pid, gallery_add adv
    # do the test
    # noted that if the probe comes from the camera of target, if would cause error when we change the id
    # because there is no correct img in gallery matched to the probe
    ...
    result_target = eval_without_noise(net, adv_probe_imgs, adv_probe_infos, gallery_imgs_add, gallery_infos_add)

    return [result_untarget, result_target]


# eval the CMC/mAP for both benign/adv settings, be called by single_evaluate()
# noted that adv images should be loaded from the dir, rather than generated by the mask/trans/noise
...
#eval the CMC/mAP for only adv settings
#noted that adv images are generated by the mask/trans/noise on the raw images
def eval_with_noise(probe, probe_infos, gallery, gallrey_infos, noise, mask, trans):
    pass
...
def adv(net, mask, generator, batch_size):
    ...
    target_tf = tf.placeholder(shape=(None, 550, 220, 3), dtype='float32')
    ...
    x0 = tf.image.resize_images(target_tf, [224, 224])
    ...
    feature0 = tf.squeeze(net(x0),axis=[1,2])
    ...
    feature1_norm_ob1 = tf.tile(tf.expand_dims(feature1_norm[0,:],0), multiples=[batch_size,1])
    feature2_norm_ob1 = tf.tile(tf.expand_dims(feature2_norm[0,:],0), multiples=[batch_size,1])

    dist1 = tf.reduce_sum(tf.multiply(feature0_norm, feature1_norm_ob1))
    dist2 = tf.reduce_sum(tf.multiply(feature0_norm, feature2_norm_ob1))
    ...
    tv =  total_variation(noise_patch)
    total_loss = -(dist1 + dist2) + weights_untarget*dist3 + weights_TV*tv

    oft1 = tf.reduce_sum(tf.multiply(feature0_norm, feature1_norm_ob1), axis=[1])
    oft2 = tf.reduce_sum(tf.multiply(feature0_norm, feature2_norm_ob1), axis=[1])
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(MAX_ITERATIONS):
            target_img, adv1_img, adv2_img, trans1, trans2 = next(generator)
...
#assume the target images belongs to a certain camera
def pair_generator(adv_imgs, adv_infos, target_imgs, target_infos, trans, batch_size):
    ...
    target_cid = target_infos[0][1]
    adv_pool = list({0,1,2}-{target_cid-7})

    while True:
        target_index = np.random.randint(0,len(target_imgs), batch_size)
        adv1_index = np.random.choice(adv_c_list[adv_pool[0]], batch_size)
        adv2_index = np.random.choice(adv_c_list[adv_pool[1]], batch_size)

        yield target_imgs[target_index], \
              adv_imgs[adv1_index], adv_imgs[adv2_index], \
              trans[adv1_index], trans[adv2_index]


def attack(net, probe_path, mask_path, adv_path, noise_path,
           adv_index, target_index, adv_id, target_id, batch_size=8):
    ...
    target_imgs = imgs[target_index]
    target_infos = [infos[i] for i in target_index]

    #generate the pair
    generator = pair_generator(adv_imgs, adv_infos, target_imgs, target_infos, trans, batch_size)
...
def evaluate(net, probe_path, gallery_path, adv_index, target_index, adv_id, target_id):
    ...
    for i in range(num_query):
        ...
        [result_untarget, result_target] = \
            single_evaluate(net, probe_path, gallery_path, tmp_adv_index, target_index, adv_id, target_id)
...
if __name__ == '__main__':
    ...
    target_id = 1505
    target_index = np.array([216,218,220,222])

    # test the bobo set
    #single_evaluate(net, probe_path, gallery_path, adv_index, target_index, adv_id, target_id)

    #generate adv, saved in adv_path
    attack(net, probe_path, mask_path, adv_path, noise_path,
           adv_index, target_index, adv_id, target_id, batch_size=8)

    # test the adv acc
    #single_evaluate(net, adv_path, gallery_path, adv_index, target_index, adv_id, target_id)
    evaluate(net, adv_path, gallery_path, adv_index, target_index, adv_id, target_id)
