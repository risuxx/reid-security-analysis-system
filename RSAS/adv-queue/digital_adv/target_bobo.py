...
#eval the CMC/mAP for adv settings
def single_evaluate(net, probe_path, gallery_path, adv_index, target_index, adv_id, target_id, graph, res):
    ...
    # do the test:
    result_untarget = eval_without_noise(net, adv_probe_imgs, adv_probe_infos, gallery_imgs_add, gallery_infos_add, graph, res)
    ...
    result_target = eval_without_noise(net, adv_probe_imgs, adv_probe_infos, gallery_imgs_add, gallery_infos_add, graph, res)

    res.put((result_untarget, result_target))
    return [result_untarget, result_target]

#eval the CMC/mAP for the bobo set
def bobo_evaluate(net, probe_path, gallery_path):
    img_names = sorted(os.listdir(probe_path))
    imgs, infos = load(probe_path, img_names)
    index = np.arange(len(imgs))

    probe_index = index[::4] #only the forward
    gal_add_index = index[2::4]

    probe_imgs = imgs[probe_index]
    probe_infos = [infos[i] for i in probe_index]

    gallery_imgs_add = imgs[gal_add_index]
    gallery_infos_add = [infos[i] for i in gal_add_index]

    eval_without_noise(net, probe_imgs, probe_infos, gallery_imgs_add, gallery_infos_add)
...
def eval_without_noise(net, probe, probe_infos, gallery_imgs_add, gallery_infos_add, graph,res):
    # probeh和gallery_imgs_add都是经过预处理的
    # init = tf.global_variables_initializer()
    with graph.as_default():
        x = net(tf.convert_to_tensor(gallery_imgs_add))
        y = net(tf.convert_to_tensor(probe))

    with tf.Session(graph=graph) as sess:
        # sess.run(init)
        net.load_weights('../baseline_dis/market-pair-pretrain-withoutwarp.h5')
        gallery_add_f = sess.run(x)
...
def adv(graph, net, mask, generator, batch_size, res, adv_index, adv_path,mask_path,probe_path):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            # 将iteration, dist1_np+dist2_np, dist3_np, tv_np, peak_np, total_loss_np 放入queue中
            res.put((iteration, dist1_np+dist2_np, dist3_np, tv_np, peak_np, total_loss_np))
...
def attack(graph, res, net, probe_path, mask_path, adv_path, noise_path,
...
def evaluate(net, probe_path, gallery_path, adv_index, target_index, adv_id, target_id, res, graph):
    ...
    for i in range(num_query):
        ...
        [result_untarget, result_target] = \
            single_evaluate(net, probe_path, gallery_path, tmp_adv_index, target_index, adv_id, target_id, graph, res)
...
if __name__ == '__main__':
    ...
    res = queue.Queue()
    evaluate(net, adv_path, gallery_path, adv_index, target_index, adv_id, target_id, res, graph)
...
"""
    ...
    attack(res, net, probe_path, mask_path, adv_path, noise_path,
           adv_index, target_index, adv_id, target_id, batch_size=2)#batch_size=2才能运行，受机器限制
