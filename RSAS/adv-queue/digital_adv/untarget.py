...
def adv(net, mask, generator, batch_size):
    ...
    noise_patch = tf.stack([noise] * batch_size)
    ...
    with tf.Session() as sess:
        sess.run(init)
        net.load_weights('../baseline_dis/market-pair-pretrain-withoutwarp.h5')
        # net.load_weights('../baseline_dis/market-pair-pretrain.h5')
...
def attack(net, probe_path, mask_path, adv_path, noise_path, adv_id, adv_index, batch_size=4):
    ...
    # use noise to generate adv img
    # noise = np.load(os.path.join(noise_path, str(adv_id)+'_noise.npy'))
    '''
    save the adv
    '''
...
def save_adv(index, imgs_raw, trans_all, mask, noise, img_names, dir_path):
    ...
    noise_tmp = tf.contrib.image.transform(noise, transform_tf, 'BILINEAR')
...
if __name__ == '__main__':
    ...
    probe_path = '../../dataset' + '/20200114/resize'
    mask_path = '../../dataset' + '/20200114/mask'
    adv_path = '../../dataset' + '/20200114/untar_adv'

    noise_path = '../../dataset' +'/20200114/untar_noise'

    #generate adv, saved in adv_path
    # adv_id = 1502
    # adv_index = np.array([54,68, 30, 50, 20, 18, 0, 4, 8, 12, 24, 28, 32, 40, 44, 46,  58, 62, 70])
    adv_id = 2001
    adv_index = np.arange(34*2+36)
    np.random.shuffle(adv_index)

    attack(net, probe_path, mask_path, adv_path, noise_path, adv_id, adv_index, batch_size=4)
